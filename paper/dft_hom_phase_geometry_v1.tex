...
A simple geometric argument shows why a diffusion coefficient of order
unity, and in fact $D\approx 1$, is natural in the normalized units of
Eq.~\eqref{eq:motion-budget}. Consider the space of microscopic increments
$(\Delta x,\Delta\theta)$ satisfying $(\Delta x)^{2} + (\Delta\theta)^{2}=1$.
If the stepping rule samples this unit circle isotropically, then
$\Delta x$ and $\Delta\theta$ are marginally distributed with zero
mean and equal second moments,
\[
    \E[(\Delta x)^{2}] = \E[(\Delta\theta)^{2}] = \frac{1}{2}.
\]
For a single trajectory the phase increment per step therefore obeys
$\Var(\Delta\theta) = 1/2$. The relative phase between two
independently stepped trajectories $A$ and $B$ is
\[
    \Delta\theta_{AB} = \Delta\theta^{(A)} - \Delta\theta^{(B)},
\]
with variance
\[
    ...
    = 1,
\]
assuming the increments for $A$ and $B$ are uncorrelated.
Under these idealized isotropy assumptions one obtains $D=1$ exactly
for the per-step variance of the relative phase. The Monte Carlo
results of Sec.~\ref{sec:phase-diffusion}, which give
$D = 0.985 \pm 0.003$, are therefore consistent with the geometric
expectation that $D$ is fixed to unity in the normalized units by the
combination of the motion budget and isotropic sampling of the
increment space.
...
\end{equation}
For $N \ll D/\sigma_{\omega}^{2}$, bandwidth dominates (Gaussian behavior).
For $N \gg D/\sigma_{\omega}^{2}$, intrinsic diffusion dominates (exponential behavior).
...
\section{Appendix B: Simulation Methodology and Statistical Analysis}
...
All simulation code and data used in this paper will be made available
upon publication in a version-controlled public repository, including:
(i) scripts to reproduce the phase-diffusion and HOM simulation
results, (ii) CSV files containing the numerical data (variance curves,
coherence envelopes, and HOM coincidence statistics), and
(iii) plotting routines for Figs.~\ref{fig:pure_diffusion}--\ref{fig:hom_comparison}.  CSV output files include:
\begin{itemize}
...
\section{Appendix C: Beamsplitter Symmetry Constraints and Geometric Interpretation}
